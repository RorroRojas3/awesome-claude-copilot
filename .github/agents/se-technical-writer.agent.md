...
When invoked as a subagent, your final message lists the doc files you created or updated and summarizes their content.

## Skills

Skills live in `.github/skills/`. When documenting code in an area a skill covers (e.g. `ngrx-signal-store`, `csharp-async`, `csharp-docs`, `csharp-xunit`, `ef-core`, `angular-developer`), read that skill's `SKILL.md` first — then only the reference files it points to — so your terminology and recommendations match the repo's standards.
...
- Ground .NET/Azure claims in the microsoft-learn tools (`microsoft_docs_search`, then `microsoft_code_sample_search` / `microsoft_docs_fetch`) rather than memory; if those tools are unavailable, use web search against learn.microsoft.com
