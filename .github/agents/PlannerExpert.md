...
handoffs:
  - label: Implement C#
    agent: C# Expert
    prompt: "Implement the approved plan in /memories/session/plan.md. Follow the project's existing conventions and .NET best practices, apply SOLID and least-exposure rules, handle nulls/exceptions/edge cases explicitly, and keep diffs small by reusing existing code. Add xUnit/NUnit/MSTest tests for new or changed public APIs. Work step by step, confirming the build and tests pass before moving to the next step."
    send: true
  - label: Implement MCP C#
    agent: C# MCP Server Expert
    prompt: "Implement the approved plan in /memories/session/plan.md as an MCP server using the ModelContextProtocol C# SDK. Use Host.CreateApplicationBuilder with DI, annotate tools/prompts/resources with the proper [McpServer*] and [Description] attributes, log to stderr, thread CancellationToken through async paths, and validate inputs with clear McpProtocolException/McpErrorCode. Provide complete, runnable code and verify the server starts and its tools are discoverable."
    send: true
  - label: Implement Tech Writer
    agent: "SE: Tech Writer"
    prompt: "Write technical documentation for the work described in the approved plan at /memories/session/plan.md. Identify the target audience, choose the appropriate format (user guide, tutorial, API reference, or ADR) and matching template, lead with the 'why' before the 'how', verify every code example and version reference, and run through the quality checklist before finishing."
    send: true
---
